    ...
    if isinstance(out_grad, dict) and "out_entropy" in out_grad:
        ...
        if len(out_grad.shape) == 2:
            ...
            return (
                ...
                jnp.argmax(jnp.einsum("ij, kj -> jki", B_factor, out_grad), axis=0),
                jnp.argmax(jnp.einsum("ki, kj -> kij", A_factor, out_grad), axis=0),
            )
            ...
            return (
                ...
                jnp.argmax(jnp.einsum("ij, bkj -> jbki", B_factor, out_grad), axis=0),
                jnp.argmax(jnp.einsum("bki, bkj -> kbij", A_factor, out_grad), axis=0),
            )
        ...
        if len(out_grad.shape) == 2:
            ...
            return (
                ...
                jnp.argmin(jnp.einsum("ij, kj -> jki", B_factor, out_grad), axis=0),
                jnp.argmin(jnp.einsum("ki, kj -> kij", A_factor, out_grad), axis=0),
            )
        elif len(out_grad.shape) == 3:
            # THIS IS JUST NOT TESTED :/
            # A_factor.shape = (b, k, i)
            ...
            return (
                ...
                jnp.argmin(jnp.einsum("ij, bkj -> jbki", B_factor, out_grad), axis=0),
                jnp.argmin(jnp.einsum("bki, bkj -> kbij", A_factor, out_grad), axis=0),
            )
